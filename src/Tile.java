...
public class Tile implements Comparable {
    ...
    public void check() {
        if (p != null && p.type > 0 && team != p.team) {
            if (team == 0)
                team = p.team;
            else
                p.team = team;
    ...
    public boolean isConnected(Tile other) {
        return (this.up != null && this.up.id == other.id || this.upright != null && this.upright.id == other.id
                || this.downright != null && this.downright.id == other.id
                || this.down != null && this.down.id == other.id
                || this.downleft != null && this.downleft.id == other.id
                || this.upleft != null && this.upleft.id == other.id);
    }
    ...
    private Polygon makePolygon(double[] c) {
        ...
        for (int i = 0; i < c.length; i++) {
            p.addPoint(
                    (int) (polygon.xpoints[(int) c[i]] * (c[i] % 1)
                            + polygon.xpoints[(int) c[i] + 1] * (1 - (c[i] % 1))),
                    (int) (polygon.ypoints[(int) c[i]] * (c[i] % 1)
                            + polygon.ypoints[(int) c[i] + 1] * (1 - (c[i] % 1))));
        }
        return p;
    }
    ...
    public void drawMe(Graphics2D g, boolean isFocus, boolean showMove) {
        ...
        // g.getFontMetrics().stringWidth(String.valueOf(distance))/2), (int) (y +
        // g.getFontMetrics().getMaxAscent()/2 - g.getFontMetrics().getMaxDescent()/2));

        // g.drawString(String.valueOf(id), (int) (x -
        // g.getFontMetrics().stringWidth(String.valueOf(id))/2), (int) (y +
        // g.getFontMetrics().getMaxAscent()/2 - g.getFontMetrics().getMaxDescent()/2));
    }
